...
type
Tfrm_AceitarSocorro = class(TForm)
lHeader: TLayout;
...
var
frm_AceitarSocorro: Tfrm_AceitarSocorro;
API_KEY: string;
implementation
{$R *.fmx}
...
procedure Tfrm_AceitarSocorro.btnMenuClick(Sender: TObject);
begin
Menu.Show;
end;
procedure Tfrm_AceitarSocorro.FormCreate(Sender: TObject);
var
...
procedure Tfrm_AceitarSocorro.minhaLocalizacaoLocationChanged(Sender: TObject; const OldLocation, NewLocation: TLocationCoord2D);
begin
UpdateMap(NewLocation.Latitude, NewLocation.Longitude);
end;

procedure Tfrm_AceitarSocorro.timerTimer(Sender: TObject);
var idRecebido: Integer;
begin
...
// updateMap(minhaLocalizacao.Sensor.Latitude, minhaLocalizacao.Sensor.Longitude);
    ...
    begin
    idRecebido :=  Connection.FDQuery.FieldByName('id').AsInteger;
      if ( idRecebido > 0) then
      begin
          Connection.FDQuery.Close;
          Connection.FDQuery.SQL.Clear;
          Connection.FDQuery.SQL.Add('UPDATE pedido_socorro SET status = ''EM_ANDAMENTO'' WHERE id = :id');
          Connection.FDQuery.ParamByName('id').AsInteger := idRecebido;
...
procedure Tfrm_AceitarSocorro.UpdateMap(Latitude, Longitude: Double);
var
posicao: TMapCoordinate;
marcador: TMapMarkerDescriptor;
URL: string;
